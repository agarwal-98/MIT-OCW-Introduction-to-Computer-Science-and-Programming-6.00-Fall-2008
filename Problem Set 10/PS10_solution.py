...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        False otherwise
        """   
        word_count = getFrequencyDict(letters)
        for letter in letters:
            if letter in self.handDict.keys() and word_count[letter] <= self.handDict[letter]:
                continue 
            else:
                return False
        return True     
    ...
    def __eq__(self, other):
        ...
        if len(self.handDict) == len(other.handDict):
            for letter in self.handDict:
                if letter in other.handDict and self.handDict[letter] == other.handDict[letter]:
                    continue
                else:
                    return False
        else:
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist.getList():
            if self.getHand().containsLetters(word):
                if getWordScore(word) > best_value:
                    best_value = getWordScore(word)
                    best_word = word 
        if  best_value > 0:
            return best_word
        else:
